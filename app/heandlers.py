...
async def cb_start_monitoring(callback: CallbackQuery, bot):
    ...
    await bot.delete_message(chat_id=callback.from_user.id, message_id=callback.message.message_id)
    await bot.send_message(chat_id=callback.from_user.id, text='Начинаю мониторинг выдачи по указанным фильтрам')

    radius, price, year, manufacturer = search_filter.get("radius"), \
        search_filter.get("price"), search_filter.get("year"), search_filter.get("manufacturer")
    url = await get_url(radius, price, year, manufacturer)
    ...
    while True:
        ...
        if new_car:
            for car in new_car:
                await bot.send_message(callback.from_user.id, f'По заданным фильтрам есть новое объявление: \n '
